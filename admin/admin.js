...
let supabase=null,editingId=null,coverPath=null,coverUrl=null;
function toast(msg){const t=$('#toast');t.textContent=msg;t.classList.add('show');setTimeout(()=>t.classList.remove('show'),2200)}
function loginMessage(msg,type='error'){const el=$('#loginMessage');el.textContent=msg;el.className='notice '+type}
function slugify(v){return v.normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().trim().replace(/[^a-z0-9]+/g,'-').replace(/^-+|-+$/g,'').slice(0,100)}
function esc(v){return String(v??'').replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]))}
function show(view){$$('.view').forEach(x=>x.classList.toggle('active',x.id===view));$$('.nav[data-view]').forEach(x=>x.classList.toggle('active',x.dataset.view===view));const titles={dashboard:'Visão geral',news:'Notícias',editor:editingId?'Editar notícia':'Nova notícia'};$('#pageTitle').textContent=titles[view]||'Painel';if(view==='news')loadNews()}
$$('.nav[data-view]').forEach(b=>b.onclick=()=>show(b.dataset.view));$$('[data-go]').forEach(b=>b.onclick=()=>show(b.dataset.go));
function setStatus(text,ok=false){$('#connectionStatus').textContent=text;$('#connectionStatus').previousElementSibling?.classList.toggle('ok',ok)}
async function requireAdmin(){
  if(!supabase){$('#app').hidden=true;$('#login').classList.add('active');setStatus('Erro de configuração');loginMessage('Não foi possível conectar ao sistema. Recarregue a página.');return false}
  const {data:{session},error:sessionError}=await supabase.auth.getSession();
  if(sessionError){$('#app').hidden=true;$('#login').classList.add('active');setStatus('Erro de conexão');loginMessage('Não foi possível verificar a sessão. Tente novamente.');return false}
  ...
  if(error){console.error('Admin authorization error:',error);await supabase.auth.signOut();$('#login').classList.add('active');$('#app').hidden=true;setStatus('Erro de autorização');loginMessage('Não foi possível verificar seu acesso de administrador. Tente novamente.');return false}
...
$('#title').oninput=()=>{if(!$('#slug').dataset.manual)$('#slug').value=slugify($('#title').value);if(!$('#seoTitle').value)$('#seoTitle').value=$('#title').value.slice(0,70)};$('#slug').oninput=e=>e.target.dataset.manual='1';$('#coverFile').onchange=updateCover;function updateCover(){const f=$('#coverFile').files[0],p=$('#coverPreview');if(f){p.style.backgroundImage=`url(${URL.createObjectURL(f)})`;p.querySelector('span').style.display='none'}else if(coverUrl){p.style.backgroundImage=`url(${coverUrl})`;p.querySelector('span').style.display='none'}else{p.style.backgroundImage='none';p.querySelector('span').style.display='block'}}
